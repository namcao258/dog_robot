"""
Trajectory Controller for Nova-SM3 - Front Right Leg (FR)
Same as FL trajectory controller but mirrored for right leg

Key differences from FL (Front Left):
- Y coordinate: -60mm (outward to right, mirrored)
- θ1 (hip): Mirrored (negated)
- IK: Mirror y input and theta1 output

Author: FR Leg Controller
Date: 2025-12-13
"""
...
class TrajectoryGeneratorFR:
    """Generate trajectories for Front Right leg"""

    @staticmethod
    def walking_gait(t, step_height=50, step_length=100, step_duration=1.0, y_offset=-60):
        """
        Walking gait for FR leg
    ...
    def circle(t, radius=30, center=(-52.5, -60, -222.93), period=2.0):
        """Circular trajectory for FR leg"""
...
class TrajectoryControllerFR:
    """Trajectory controller GUI for Front Right leg"""

    def __init__(self, root):
        self.root = root
        self.root.title("Nova-SM3 Trajectory Controller - Front Right Leg (FR)")
    ...
    def setup_ui(self):
        ...
        # Left panel - Controls
        control_frame = ttk.LabelFrame(main_frame, text="FR Leg Controls", padding=10)
        ...
        # Initial config
        init_frame = ttk.LabelFrame(control_frame, text="Initial Config (FR - Right Leg)", padding=10)
        ...
        self.ax_3d.set_title("FR - 3D Trajectory & Leg", fontweight='bold')
        ...
        self.ax_xz.set_title("FR - XZ Projection", fontweight='bold')
        ...
        self.ax_angles.set_title("FR - Joint Angles vs Time", fontweight='bold')
        ...
        self.ax_xy.set_title("FR - XY Projection", fontweight='bold')
    ...
    def select_best_ik_solution_FR(self, x, y, z, current_angles):
        """
        Select best IK solution for FR (RIGHT leg)
        ...
        This is because L1 points in -Y direction for FR, opposite to FL!
        """
    ...
    def generate_trajectory(self):
        ...
        traj_gen = TrajectoryGeneratorFR()
        ...
        for t in self.time_array:
            ...
            # Calculate IK for FR (right leg with mirroring)
            result = self.select_best_ik_solution_FR(x, y, z, self.current_joint_angles)
            if result:
    ...
    def plot_trajectory(self):
        ...
        self.ax_3d.set_ylabel("Y (mm)")
        self.ax_3d.set_zlabel("Z (mm)")
        self.ax_3d.set_title("FR - 3D Trajectory", fontweight='bold')
        self.ax_3d.legend()
        ...
        self.ax_angles.set_xlabel("Time (s)")
        self.ax_angles.set_ylabel("Angle (degrees)")
        self.ax_angles.set_title("FR - Joint Angles", fontweight='bold')
        self.ax_angles.grid(True, alpha=0.3)
    ...
    def update_display(self, time_index):
        ...
        # Calculate leg positions for FR (RIGHT leg)
        # IMPORTANT: FR has L1 pointing in -Y direction (opposite to FL)
